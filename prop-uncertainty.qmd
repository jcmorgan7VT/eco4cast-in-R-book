...
inputs <- inputs_all |> 
    ...
    summarize(prediction = median(prediction, na.rm = TRUE), .by = c("datetime", "variable")) |> 
...
#Set initial conditions
output <- array(NA, dim = c(length(sim_dates), ens_members, 12)) #12 is the number of outputs
output[1, , 1] <- rnorm(ens_members, 5, 0.5)
output[1, , 2] <- rnorm(ens_members, 140, 10)
output[1, , 3] <- rnorm(ens_members, 140, 20)
...
initial_conditions_df <- output_to_df(output, sim_dates, sim_name = "initial_unc")

# Parameter calibration: likelihood methods
...
Which mean (mean = 0 or mean = 2) is more likely for the two data points?
...
This logically extends to three data points or more. We will get two log likelihood values because we are examining two "models" with three data points. What are the two "models"?
...
Now we will calculate the log likelihood of the data for each potential value of the mean.
...
We just calculated the **likelihood of the data GIVEN our model** where our model said that the data are normally distributed with specific means. We found the most likely value for a parameter in the model (the parameter being the mean of the PDF).

For use in R tools that find the minimum of a function, find the minimum of the negative log likelihood (@fig-ll2)
...
Finally, we don't have to assume that the *data* model is a normal distribution. Here is an example assuming that the data model follows a gamma distribution (@fig-data-curve-gamma).
...
**Components of a likelihood analysis that are not covered here**
...
```{r}
df <- read_csv("https://raw.githubusercontent.com/frec-5174/frec-5174-docs/main/data/soil_respiration_module_data.csv", show_col_types = FALSE)
```

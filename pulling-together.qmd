...
```{r}
ens_members <- 100
sim_dates <- seq(as_date("2023-01-07"), as_date("2023-11-15"), by = "1 day")
```
...
for(t in 2:length(sim_dates)){
  ...
  analysis <- particle_filter(forecast, obs, sim_dates, wt, fit_params)
...
```{r}
save(analysis, file = "data/PF_analysis.Rdata")
```
...
```{r}
load("data/PF_analysis.Rdata")
```

```{r}
sim_dates <- seq(as_date("2023-11-15"), length.out = 30, by = "1 day")
index <- which(analysis$sim_dates==sim_dates[1])
```

```{r}
inputs <- get_forecast_met(site = "TALL", sim_dates, use_mean = FALSE)
inputs_ensemble <- assign_met_ensembles(inputs, ens_members)
```
...
Convert forecast to a dataframe in the format required for the NEON Ecological Forecasting Challenge.
...
```{r}
sim_dates <- seq(as_date("2023-11-16"), length.out = 30, by = "1 day")
index <- which(analysis$sim_dates == sim_dates[1])
```
...
### Step 5: Assimilate new data and generate new forecast

Note that the loop starts at t = 1 because it will assimilate new data that wasn't avialable with the forecast used to initialize the forecast was generated.
...
TO DO: Add concept of "look back"

```{r}
look_back <- 7
horizon <- 30
sim_dates <- seq(as_date("2023-11-11"), length.out = look_back + horizon, by = "1 day")
inputs_past <- get_historical_met(site = "TALL", sim_dates[1:look_back], use_mean = FALSE)
inputs_future <- get_forecast_met(site = "TALL", sim_dates[(look_back+1):horizon], use_mean = FALSE)
